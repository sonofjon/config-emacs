...
(ert-deftest test-aj8-buffer-and-file-conversion ()
  ...
  (with-temp-file-with-content test-file "content"
    (let ((buffer (find-file-noselect test-file)))
      (unwind-protect
          (progn
            (should (string-equal (aj8/gptel-tool-aj8-buffer-to-file (buffer-name buffer))
                                  (expand-file-name test-file)))
            (should (string-equal (aj8/gptel-tool-aj8-file-to-buffer test-file)
                                  (buffer-name buffer)))
            (should-error (aj8/gptel-tool-aj8-buffer-to-file "*scratch*") :type 'error)
            (should-error (aj8/gptel-tool-aj8-file-to-buffer "/non/existent/file.tmp") :type 'error))
        (kill-buffer buffer)))))
...
(ert-deftest test-my-buffer-modification-tools ()
  ...
  (with-temp-buffer-with-content "*test-modify*" "Line 1\nLine 3"
    ...
    ;; Insert
    (aj8/gptel-tool-my-insert-into-buffer "*test-modify*" "Line 2\n" 2)
    (should (string-equal (buffer-string) "Line 1\nLine 2\nLine 3\nLine 4"))
    ;; Modify
    (aj8/gptel-tool-my-modify-buffer "*test-modify*" "New Content")
...
(ert-deftest test-aj8-edit-buffer ()
  ...
  (with-temp-buffer-with-content "*test-edit*" "hello world\nhello universe"
    ...
    (should-error (aj8/gptel-tool-aj8-edit-buffer "*test-edit*" "non-existent" "foo") :type 'error)
    (should-error (aj8/gptel-tool-aj8-edit-buffer "*test-edit*" "hello" "hi") :type 'error)))
...
(ert-deftest test-aj8-apply-buffer-edits ()
  ...
  (with-temp-buffer-with-content "*test-apply-edits*" "Line one.\nLine two.\nLine three."
    (let ((edits '((:line-number 3 :old-string "three" :new-string "THREE")
      ...
      (aj8/gptel-tool-aj8-apply-buffer-edits "*test-apply-edits*" edits)
  ...
  (with-temp-buffer-with-content "*test-review*" "Line one.\nLine two."
    (let ((edits '((:line-number 1 :old-string "one" :new-string "ONE")))
      ...
      (cl-letf (((symbol-function 'ediff-buffers) (lambda (b1 b2) (setq ediff-called t))))
        (aj8/gptel-tool-aj8-apply-buffer-edits-with-review "*test-review*" edits))
      (should ediff-called)
...
(ert-deftest test-my-read-file-section ()
  ...
  (with-temp-file-with-content test-file "Line 1\nLine 2\nLine 3\nLine 4\nLine 5"
    ...
    (should (string-equal (aj8/gptel-tool-my-read-file-section test-file)
                          "Line 1\nLine 2\nLine 3\nLine 4\nLine 5"))
    ;; Read a section
    (should (string-equal (aj8/gptel-tool-my-read-file-section test-file 2 4)
                          "Line 2\nLine 3\nLine 4"))
    ;; Read from start
    (should (string-equal (aj8/gptel-tool-my-read-file-section test-file nil 2)
                          "Line 1\nLine 2"))
    ;; Read to end
    (should (string-equal (aj8/gptel-tool-my-read-file-section test-file 4)
                          "Line 4\nLine 5"))))
...
(ert-deftest test-aj8-file-modification ()
  ...
  (with-temp-file-with-content test-file "Line 1\nLine 3"
    ...
    (should (string-equal (with-temp-buffer (insert-file-contents test-file) (buffer-string))
                          "Line 1\nLine 3\nLine 4"))
    ;; Insert
    (aj8/gptel-tool-aj8-insert-into-file test-file "Line 2\n" 2)
    (should (string-equal (with-temp-buffer (insert-file-contents test-file) (buffer-string))
                          "Line 1\nLine 2\nLine 3\nLine 4"))
    ;; Edit
    (aj8/gptel-tool-aj8-edit-file test-file "Line 4" "Line FOUR")
    (should (string-equal (with-temp-buffer (insert-file-contents test-file) (buffer-string))
                          "Line 1\nLine 2\nLine 3\nLine FOUR"))))
...
(ert-deftest test-aj8-apply-file-edits ()
  ...
  (with-temp-file-with-content test-file "Line one.\nLine two.\nLine three."
    (let ((edits '((:line-number 3 :old-string "three" :new-string "THREE")
                   (:line-number 1 :old-string "one" :new-string "ONE"))))
      (aj8/gptel-tool-aj8-apply-file-edits test-file edits)
      (should (string-equal (with-temp-buffer (insert-file-contents test-file) (buffer-string))
                            "Line ONE.\nLine two.\nLine THREE.")))))
...
(ert-deftest test-aj8-apply-file-edits-with-review ()
  ...
  (with-temp-file-with-content test-file "Line one.\nLine two."
    (let ((edits '((:line-number 1 :old-string "one" :new-string "ONE")))
          (ediff-called nil))
      (cl-letf (((symbol-function 'ediff-buffers) (lambda (b1 b2) (setq ediff-called t))))
        (aj8/gptel-tool-aj8-apply-file-edits-with-review test-file edits))
      (should ediff-called)
...
(ert-deftest test-aj8-project-root-and-buffers ()
  ...
  (with-temp-project
    (let ((root (file-truename default-directory)))
      ;; Test get root
      (should (string-match-p (regexp-quote root) (aj8/gptel-tool-my-project-get-root)))
      ;; Test get open buffers
      (let ((buf (find-file-noselect (expand-file-name "src/code.el"))))
        (unwind-protect
            (let ((open-buffers (aj8/gptel-tool-my-project-get-open-buffers)))
...
(ert-deftest test-aj8-project-find-and-search ()
  ...
  (with-temp-project
    ...
    (when (or (executable-find "rg") (and (executable-find "git") (file-directory-p ".git")))
      (let ((results (aj8/gptel-tool-my-project-search-content "some text data")))
