...
(use-package eglot
  ...
  ;;   completion-at-point-functions. Since eglot returns an empty list when
  ;;   LSP has no candidates (rather than nil), it blocks subsequent capfs
  ;;   from running. Select one of the solutions below.  See aj8-lisp.el for
  ...
  (display-buffer-alist
   `(;;
     ...
     ("^\\*\\(Bookmark List\\|Benchmark Init Results.*\\|ChatGPT.*\\|Claude.*\\|claude:.*\\|claude-code.*\\|Deepseek.*\\|Embark Collect:.*\\|Gemini.*\\|gptel-agent:.*\\|Occur\\|.*Output\\|Semantic SymRef\\|devdocs\\|eldoc\\|package update results\\|tex-shell\\)\\*"
      ...
      (slot . ,aj8/side-window-slot-top)
      (window-parameters . ((no-delete-other-windows . t))))
     ("^\\(Claude Agent.*\\|Codex Agent.*\\|Copilot Agent.*\\|Gemini Agent.*\\|gptel-.*\\.\\(org\\|md\\)\\)"
      (display-buffer-in-side-window)
      (window-width . ,aj8/side-window-width-dynamic)
      (side . right)
      (slot . ,aj8/side-window-slot-top)
...
(use-package agent-shell-manager
  ...
  :config
  ;; Open manager when an agent-shell buffer is created
  (add-hook 'agent-shell-mode-hook
            (lambda ()
              (run-with-idle-timer 0.1 nil
                                   #'aj8/agent-shell-manager-show))
            90))
...
(use-package gptel
  ...
  ;;   magic-mode-alist rather than auto-mode-alist, since markdown-ts-mode
  ;;   already claims ".md" there via its own :mode keyword, which would
  ;;   otherwise route these files to markdown-ts-mode first.
...
Elisp code explicitly in arbitrary buffers.")
  ...
  ;; Completion at point functions for various modes
  ;;   See aj8-lisp.el for function definitions.
